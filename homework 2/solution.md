...
Все коммуникации между сервисами синхронные за исключением исключений: выплаты.
Выплаты не обязательно ждут выполнения заказов, а просто активируются раз в когда-то.

Все остальные действия выполняются синхронно, в особенности работа со складом и система матчинга. В этих случаях сервисы выполняют работу последовательно из-за temporal coupling, и следующему сервису нужен результат работы предыдущего.

~~Единственное, что не понятно - это как превратить тетрадку в микросервис, но это уже проблема разработчиков~~
